...
if($operation == 'patch' || $operation == 'cross') {
	...
	if($step == 1) {
		...
		showtablerow('', '', array($lang['founder_upgrade_store_directory'].'./data/update/Kejian.TV X'.$version.' Release['.$release.']'));
		...
		showtablerow('', 'colspan="2"', $lang['founder_upgrade_download_file'].' ./data/update/Kejian.TV X'.$version.' Release['.$release.']'.'');
		showtablerow('', 'colspan="2"', $lang['founder_upgrade_backup_file'].' ./data/back/Kejian.TV '.DISCUZ_VERSION.' Release['.DISCUZ_RELEASE.']'.$lang['founder_upgrade_backup_file2']);
		...
		if(!$_GET['startupgrade']) {
			...
			foreach($updatefilelist as $updatefile) {
				...
				$backfile = DISCUZ_ROOT.'./data/back/Kejian.TV X'.substr(DISCUZ_VERSION, 1).' Release['.DISCUZ_RELEASE.']/'.$updatefile;
				if(is_file($destfile)) {
		...
		foreach($updatefilelist as $updatefile) {
			$srcfile = DISCUZ_ROOT.'./data/update/Kejian.TV X'.$version.' Release['.$release.']/'.$updatefile;
			if($confirm == 'ftp') {
				$destfile = $updatefile;
			} else {
				$destfile = DISCUZ_ROOT.$updatefile;
		...
		if($upgradeinfo['isupdatedb']) {
			...
			foreach($dbupdatefilearr as $dbupdatefile) {
				$srcfile = DISCUZ_ROOT.'./data/update/Kejian.TV X'.$version.' Release['.$release.']/'.$dbupdatefile;
		...
		$file = DISCUZ_ROOT.'./data/update/Kejian.TV X'.$version.' Release['.$release.']/updatelist.tmp';
		@unlink($file);
