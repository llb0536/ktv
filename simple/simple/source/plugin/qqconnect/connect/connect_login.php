...
if($op == 'init') {
	...
	if($_G['uid']) {
		...
		} else { // debug 当前登录的课件系统账号并没有绑定任何QQ号，则可以绑定当前的这个QQ号
		...
		if($connect_member) { // debug 此分支是用户直接点击QQ登录，并且这个QQ号已经绑好一个课件系统账号了，将直接登进课件系统了
		...
		} else { // debug 此分支是用户直接点击QQ登录，并且这个QQ号还未绑定任何课件系统账号，将将跳转到一个新页引导用户注册个新课件系统账号或绑一个已有的课件系统账号
