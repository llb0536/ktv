...
function removeindexheats() {
	return confirm('您确认要把此课件从热点课件中移除么？');
}
...
function loadData(quiet, formobj) {
	...
	if(!quiet && !confirm('此操作将覆盖当前课件内容，确定要恢复数据吗？')) {
		return;
	}
...
function checkForumnew(fid, lasttime) {
	...
	x.get('forum.php?mod=ajax&action=forumchecknew&fid=' + fid + '&time=' + lasttime + '&inajax=yes', function(s){
		if(s > 0) {
			...
			var checknew = {'tid':'', 'thread':{'common':{'className':'', 'val':'<a href="javascript:void(0);" onclick="ajaxget(\'forum.php?mod=ajax&action=forumchecknew&fid=' + fid+ '&time='+lasttime+'&uncheck=1&inajax=yes\', \'forumnew\');">有新回复的课件，点击查看', 'colspan': colspan }}};
