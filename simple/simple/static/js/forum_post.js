...
function validate(theform) {
	...
	if(UPLOADSTATUS == 0) {
		...
		showDialog('您有正在上传的附件，请稍候，上传完成后课件将会自动上传...', 'notice');
		AUTOPOST = 1;
		return false;
	}
	if(isfirstpost && $('adddynamic') != null && $('adddynamic').checked && $('postsave') != null && isNaN(parseInt($('postsave').value)) && ($('readperm') != null && $('readperm').value || $('price') != null && $('price').value)) {
		if(confirm('由于您设置了阅读权限或出售课件，您确认还转播给您的听众看吗？') == false) {
			return false;
		}
	}
...
function getreplycredit() {
	...
	if(real_reply_credit > userextcredit) {
		$('replycredit').innerHTML = '<b class="xi1">学习笔记奖励积分总额过大('+real_reply_credit+')</b>';
	} else {
