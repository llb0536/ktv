var Asks = {
    ...
    redirected_tip : function(title, id, type, rf_id){
        if(title == undefined){
            ...
            var html = '<div id="redirected_tip"><div class="container notice_message '+type+'">';
            if (type==="rf") {
                html += '从问题 <a href="/asks/'+id+'?nr=1">'+title+'</a> 跳转而来';
            } else {
                html += '已被关联到相似问题： <a href="/asks/'+id+'?nr-1&rf='+rf_id+'">'+title+'</a>';
            }
            html += '</div></div>';
            
            /*
            label_text = "已被关联到相似问题："
            ask_link = "/asks/" + id + "?nr=1&rf=" + rf_id;
            if(type == "rf"){
                label_text = "从问题：";
                ask_link = "/asks/" + id + "?nr=1";
            }
            html = '<div id="redirected_tip"><div class="container notice_message '+type+'">';
    ...
    vote_callback : function(id, vtype, new_up_count, new_down_count,new_who){
        ...
        var up_num = $(".votes_up_num",answer);
        var down_num = $(".votes_down_num",answer);
        ...
        up_num.text(new_up_count);
        down_num.text(new_down_count);
    ...
    addAsk: function(){
        ...
        ask_search_text = $("#searchInput").val() != "搜索求助、学术疑问" ? $("#searchInput").val() : "";
