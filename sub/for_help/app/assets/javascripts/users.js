var Users = {
  visit : function(page, id){
      $.post("/home/visit",{page:page,user_id:id}, function(res){
  });
  },
  visit2 : function(page){
      $.post("/home/visit",{page:page,user_id:"nil"}, function(res){
  });
  },
  follow : function(el, id, small){
    ...
    $.get("/users/"+id+"/follow",{}, function(res){
            if(res=="2"){
                $(el).replaceWith('<a onclick="return Users.unfollow(this, \''+ id +'\',\''+small+'\');" class="'+(small == 'small'?'nBtn nBtndubleFocus':'bBtn bBtndoubleFocus')+'"></a>');
            }else{
  ...
  userLogin: function(){
    var lhtml = [];
    lhtml.push('<header>欢迎使用课件交流系统账号登录求助 <a class="close" href="javascript:void(0);"></a></header>');
    ...
    lhtml.push('<input style="width:18px;vertical-align:-2px" type="checkbox" class="noBorder" name="isautologin" value="1" />记住登录状态</dd>');
    ...
    lhtml.push('<div class="goLog mt20">没有课件交流系统账号？<a href="javascript:void(0);" onclick="Users.userReg()">立即注册</a></div>');
    lhtml.push('</footer>');
...
//      }
  ...
  userReg: function(){
    ...
    lhtml.push('<dt></dt>');
    lhtml.push('<dd class="clear"><div class="details"><input type="checkbox" class="noBorder accept" name="accept" checked="checked" style="width:20px;" />我接受 <a target="_blank" href="/agreement">求助用户协议</a> 和 <a target="_blank" href="http://jobseeker.kejian.tv/zhaopin/aboutus/secrecy.html">课件交流系统隐私协议</a></div></dd>');
    lhtml.push('</dl>');
    lhtml.push('</form></section>');
    lhtml.push('<footer>');
    lhtml.push('<div class="btnNormalGreen bold mt20 reg"><span>&nbsp;注 册&nbsp;</span></div>');
    lhtml.push('<div class="goLog mt20">已有课件交流系统账号？<a href="javascript:void(0);" onclick="Users.userLogin()">直接登录</a></div>');
    lhtml.push('</footer>');
    $.facebox({ html : lhtml.join(""), overlay : false });
