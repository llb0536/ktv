...
protected
  ...
  def api_super_client?
    Setting.trusted_client_ips.include?(request.remote_ip) or request.remote_ip.starts_with?('192.168') or request.remote_ip.starts_with?('172.30')
  end
  def render_this!
  ...
  def oauth_authorized
    ...
    if (@token.nil? or @token.blocked?) and @current_client.nil?
      ...
      @resource_owner_uri = @token ? @token.resource_owner_uri : "http://wendao.kejian.tv/users/#{@current_client.user.slug}"
