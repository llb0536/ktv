...
class Cpanel::AsksController < CpanelController
  ...
  def index
    ...
    if params[:spams_count].to_i==1
      ...
      @asks = @asks.where(:spams_count.gte => Setting.ask_spam_max)
    elsif params[:spams_count].to_i==3
      @asks = @asks.where(:good_ask=>true)
    elsif params[:spams_count].to_i==4
      @asks = @asks.where(:redirect_ask_id.ne=>nil)
    elsif params[:spams_count].to_i==5
      @asks = @asks.any_of({:to_user_id.ne=>nil},{:to_user_ids.ne=>nil})
    end
  ...
  def update
    ...
    if params[:ask_type].to_i==1
      params[:ask][:good_ask]=false
      if params[:ask][:spams_count].to_i>7
        params[:ask][:spams_count]=0
      end
    elsif params[:ask_type].to_i==2
      params[:ask][:good_ask]=true
      if params[:ask][:spams_count].to_i>7
        params[:ask][:spams_count]=0
      end
    elsif params[:ask_type].to_i==3
      params[:ask][:good_ask]=false
      if params[:ask][:spams_count].to_i<8
        params[:ask][:spams_count]=8
      end
    end
    if params[:ask][:redirect_ask_id].blank? 
      params[:ask][:redirect_ask_id]=nil
    end
  ...
  def deal_asks
    if !params[:choose_asks].blank? and !params[:deal_action].blank?
      ...
      if params[:deal_action].to_i==1
        ...
        asks.each do |a|
          a.update_attribute(:good_ask,true)
        end
        notice="Asks 处理成功。"
      end
    else
