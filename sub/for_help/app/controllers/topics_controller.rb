...
class TopicsController < ApplicationController
  ...
  def show
    ...
    @asks = Ask.nondeleted.where(:topics => name).normal
    @topic_unanswered_asks_count = @asks.unanswered.count
    if params[:filter]=='new'
      @asks = @asks.unanswered
    end
    if params[:filter]=='search' and !params[:q].blank?
      @asks = @asks.where(:title=>/#{params[:q]}/)
    end
    @asks = @asks.recent.paginate(:page => params[:page], :per_page => @per_page)
