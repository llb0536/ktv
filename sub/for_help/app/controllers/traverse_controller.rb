...
=end
      ...
      @asks  = Ask.where(:_id.in=>Redis::Search.query("Ask",params[:q],:limit=>100).collect{|topic_hash| topic_hash['id']}).desc('answers_count').desc('created_at').to_a
      ...
      unless ask_ids.empty?
        @asks += Ask.where(:_id.in=>ask_ids).desc('answers_count').desc('created_at').to_a
      end
