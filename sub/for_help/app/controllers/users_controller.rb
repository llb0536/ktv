...
class UsersController < ApplicationController
  ...
  before_filter :get_view, :except => [:auth_callback,:asked_to]
  
  def get_view
    if current_user and @user.id==current_user.id
      @count=@user.to_me_ask_ids.count
    end
  end
  ...
  def answered
    @per_page = 10
    @asks = Ask.normal.nondeleted.any_in(_id:@user.answered_ask_ids).recent
  ...
  def asked_to
    @per_page = 10
    if current_user and @user.id==current_user.id
      @user.update_attribute(:to_me_ask_ids,[])
    end
    ask_ids = Ask.normal.asked_to(@user.id).nondeleted.map{|x|x.id}
    if params[:filter]=='new'
      ask_ids = ask_ids-@user.answered_ask_ids
    end
    @asks = Ask.any_in(:_id=>ask_ids).recent
    .paginate(:page => params[:page], :per_page => @per_page)
  ...
  def show
    ...
    unless params[:force]
      if @user and (!current_user|| @user.id!=current_user.id)
        if @user.is_expert and @logs.blank?
          redirect_to "/users/#{@user.slug}/asked_to"
        end
      end
    end
  end
  ...
  def asked
    @per_page = 10
    @asks = @user.asks.normal.nondeleted.recent
    .paginate(:page => params[:page], :per_page => @per_page)
  ...
  def follow
    ...
    if current_user.followed_each?(@user)
      render :text => "2"
    else
      render :text => "1"
    end
