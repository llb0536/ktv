...
module AsksHelper
  ...
  def ask_search_title_tag(ask, options = {})
    class_name = options[:class] || ""
    prefix = ""
    if !ask.to_user.blank?
      prefix = "请问#{ask.to_user.name}："
    ...
    if options[:q]
      title=h(ask.title).gsub(options[:q],"<span style=\"color:red;\">#{options[:q]}</span>")
      raw "<a href=\"/asks/#{ask.id}\" class=\"#{class_name}\">#{h(prefix)}#{title}</a>"
    else
      raw "<a href=\"/asks/#{ask.id}\" class=\"#{class_name}\">#{h(prefix)}#{h(ask.title)}</a>"
    end
  end  
