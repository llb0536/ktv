...
class Answer
  ...
  def msg_center_action
    if self.ask.user_id!=self.user_id  
      send_to_msg_center({
          "SourceId"=>"",
          "MsgType"=>30,
          "MsgSubType"=>3010,
          ...
          "SenderUrl"=>"http://wendao.kejian.tv/users/#{self.user.slug}",
          "SendContent"=>"<P>回答了你的问题“<a href=\"http://wendao.kejian.tv/asks/#{self.ask.id}\">#{self.ask.title}</a>”。</P>",
          "SendContentUrl"=>"",
          "OperateUrl"=>""
        })
    end
      ...
      if item.id!=self.user_id
        send_to_msg_center({
            ...
            "Sender"=>"#{self.user.name}",
            "SenderUrl"=>"http://wendao.kejian.tv/users/#{self.user.slug}",
            "SendContent"=>"<P>回答了你关注的问题“<a href=\"http://wendao.kejian.tv/asks/#{self.ask.id}\">#{self.ask.title}</a>”。</P>",
            "SendContentUrl"=>"",
            "OperateUrl"=>""
          })
      end
  ...
  def check_to_user
    ...
    if !ask.to_user.blank? and self.user_id != ask.to_user_id
      errors.add(:base, '这个问题是定向提问，您不是提问对象故不能回答')
    end
  end
  # 支持者
  # def up_voters
