...
class Ask
  ...
  def self.real_create(params,current_user)
    ...
    if ask.save
      if params[:onlyhim]
        user=User.where(:_id=>params[:ask][:to_user_id]).last
        if user
          user.update_attribute(:to_me_ask_ids,(user.to_me_ask_ids<<ask.id).uniq)
        end
      end
  ...
  def asynchronously_clean_me
    ...
    Util.bad_id_out_of!(User,:to_me_ask_ids,bad_ids)
    ...
    AskLog.where(:action=>"INVITE_TO_ANSWER",:target_parent_id=>self.id.to_s).each do |c|
      Notification.where(:log_id=>c._id).each do |n|
        n.update_attribute(:deleted,1)
      end
      c.delete
  ...
  field :good_ask, :type => Boolean, :default => false
  field :title
  field :body
  field :body2
  ...
  # after_save proc{
  #   Resque.enqueue(Askbody2Job,ask_id:self.id)
  # }
  def body_plain
