...
$lang = array (
  ...
  array (
    ...
    array (
      ...
      2 => '可以为个人空间、群组、版块、专题、频道设置一个二级域名的根域名。例：用户申请XXX.kejian.tv,根域名填：kejian.tv个人空间、群组设置完二级域名的根后，还需要在基本设置中开启相应的二级域名开关，用户才能在相应的地方申请绑定二级域名，同时受限于用户组的二级域名长度频道启用二级域名如果是绑定目录的，需要自已在环境中配置相关的绑定操作不建议中途将不同的根域变更为相同的根域',
    ),
  ),
  170 =>
  array (
...
由于涉及现金交易，为避免因操作不当而造成的资金损失，请在开始使用财付通积分交易功能(不包含财付通按钮功能)前，务必仔细阅读《用户使用说明书》中有关电子商务的部分，当确认完全理解和接受相关流程及使用方法后再进行相关设置。您可以设置允许用户通过现金在线支付的方式，为其交易积分账户充值，用于购买课件内容、购买用户组权限、积分转账或用户组升级等功能。财付通积分交易功能，需在“积分设置”中启用交易积分，并同时设置相应的积分策略以满足不同场合的需要。请务必正确设置您的收款财付通账号，否则将造成用户付款后积分无法实时到账，造成大量需要人工处理的订单信息。除 Kejian.TV 官方网站或官方论坛另行通知以外，Kejian.TV 提供的财付通支付服务每笔交易收取 1% 的手续费。请及时关注相关业务的最新通知，各项政策或流程的变更、调整，以 Kejian.TV 官方网站或官方论坛提供的信息为准。您使用财付通服务是建立在完全自愿的基础上，除 Kejian.TV 因主观恶意的因素造成的资金损失以外，北京课件交流系统科技有限责任公司不对因使用此功能造成的任何损失承担责任。财付通服务热线：0755-83762288-2(全天24小时服务热线)',
  ...
  array (
    ...
    array (
      ...
      15 => '只显示下级子版块',
      16 => '选择“是”将不显示本版块的课件列表、上传课件按钮等等，类似于一个分类',
      ...
      24 => '只显示下级子版块',
      25 => '选择“是”将不显示本版块的课件列表、上传课件按钮等等，类似于一个分类',
      ...
      39 => '课件默认排序字段',
      40 => '设置版块的课件列表默认按照哪个字段进行排序显示。默认为“评论时间”，除默认设置外其他排序方式会加重服务器负担',
  ...
  array (
    ...
    array (
      ...
      33 => '允许匿名上传课件',
      34 => '是否允许用户在本版匿名发表课件和评论，只要用户组或本版块允许，用户均可使用匿名上传课件功能。匿名上传课件不同于游客上传课件，用户需要登录后才可使用，版主和管理员可以查看真实作者',
      ...
      59 => '允许发送广播和动态',
      60 => '开启后默认允许用户发送广播和动态，广播是否发送用户可以在上传课件时去掉，隐私版块建议设为否。',
  ...
  array (
    ...
    array (
      ...
      6 => '启用课件老师',
      7 => '设置是否在本版块启用课件老师功能，您需要同时设定相应的分类选项，才能启用本功能',
      ...
      18 => '课件老师',
      19 => '您可以启用已有的课件老师或添加新课件老师应用于本版块，在分类很多的情况下，建议只有重要的分类使用“平板显示”方式，更多的分类使用“下拉显示”方式',
    ),
  ),
  183 =>
  array (
  ...
  array (
    ...
    array (
      ...
      4 => '版块权限',
      5 => '某权限如果全部未选则表示如下含义:　　浏览版块，全部用户组具有浏览版块课件权限；发新话题，除游客以外的用户组具有上传课件权限；发表评论，除游客以外的用户组具有评论权限；查看附件，全部用户组具有下载/查看附件权限；上传附件，除游客以外的用户组具有上传附件权限；上传图片，除游客以外的用户组具有上传图片权限',
      ...
      16 => '版块权限',
      17 => '某权限如果全部未选则表示如下含义:　　浏览版块，全部用户组具有浏览版块课件权限；发新话题，除游客以外的用户组具有上传课件权限；发表评论，除游客以外的用户组具有评论权限；查看附件，全部用户组具有下载/查看附件权限；上传附件，除游客以外的用户组具有上传附件权限；上传图片，除游客以外的用户组具有上传图片权限',
      18 => 'forums_edit_perm_formula',
      19 => '权限表达式设置',
      20 => '当您设定了权限表达式后，只有符合此表达式的会员才可以浏览本版块。如 "posts > 100 and extcredits1 > 10" 表示 "上传课件数 > 100 并且 威望 > 10"日期格式 "{Y-M-D}"，如 "{2009-10-1}"。IP 格式 "{x.x.x.x}"，既可输入完整地址，也可只输入 IP 开头，如 "{10.0.0.1}"、"{192.168.0}"',
  ...
  array (
    ...
    array (
      ...
      17 => '允许发送广播和动态',
      18 => '开启后默认允许用户发送广播和动态，广播是否发送用户可以在上传课件时去掉，隐私版块建议设为否。',
    ),
  ),
  187 =>
  array (
...
(3)SPH_RANK_NONE，禁用评分的模式，这是最快的模式。实际上这种模式与布尔搜索相同。所有的匹配项都被赋予权重1',
  ...
  array (
    ...
    array (
      ...
      2 => '课件分表功能可将 forum_thread 表(包含课件标题、创建时间等信息，但不包含课件及评论内容)分在多个数据表中。创建新的课件存档表后，可使用课件移动功能将特定课件移动到存档表中。移动到存档表中的课件，会在课件所在的版块下建立一个存档区(名称可自定义)，通过存档区可浏览存档表中的课件。存档表中的课件，只供浏览，不可评论、评分，不能进行管理操作，但可以删除和移动到非存档区。',
  ...
  array (
    ...
    array (
      ...
      10 => '允许直接上传课件',
      11 => '是否允许直接上传课件或者对评论及课件进行审核，此设置受版块审核影响，如果版块设置为不需要审核，则此设置无效',
