...
function validate(theform) {
	...
	if(UPLOADSTATUS == 0) {
		...
		showDialog('您有正在上传的附件，请稍候，上传完成后课件将会自动发表...', 'notice');
		AUTOPOST = 1;
		return false;
	}
...
function getreplycredit() {
	...
	if(real_reply_credit > userextcredit) {
		$('replycredit').innerHTML = '<b class="xi1">课件评论奖励积分总额过大('+real_reply_credit+')</b>';
	} else {
