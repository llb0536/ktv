...
$.Autocompleter.defaults = {
  ...
  defaultHTML : "先搜下题是否已存在，如果没有可以去提问哦",
  ...
  addSearch: true,
	highlight: function(value, term) {
        ...
        for (var i = 0; i < temp.length; i++){
            if (temp[i]){
                value = value.replace(new RegExp("(?![^&;]+;)(?!<[^<>]*)(" + temp[i].replace(/([\^\$\(\)\[\]\{\}\*\.\+\?\|\\])/gi, "\\$1") + ")(?![^<>]*>)(?![^&;]+;)", "gi"), "<span class='mark'>$1</span>");
            }
        }
		return value;
	},
	...
	return {
		...
		show: function() {
			...
			element.css({
				...
				top: offset.top + input.offsetHeight + options.y,  //modify 2012-2-6 by lesanc.li
				left: offset.left + options.x
			}).show();
            if(options.scroll) {
                list.scrollTop(0);
                list.css({
