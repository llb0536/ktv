...
protected
  ...
  def oauth_authorized
    ...
    if (@token.nil? or @token.blocked?) and @current_client.nil?
      ...
      @resource_owner_uri = @token ? @token.resource_owner_uri : "http://wendao.zhaopin.com/users/#{@current_client.user.slug}"
  ...
  def pagination_over(sumcount)
    ...
    info_next = "<https://api.zhaopin.com#{request.path}?page=#{@page + 1}&per_page=#{@per_page}>; rel=\"next\"" if @page < @pages
    info_last = "<https://api.zhaopin.com#{request.path}?page=#{@pages}&per_page=#{@per_page}>; rel=\"last\""
    info_first = "<https://api.zhaopin.com#{request.path}?page=1&per_page=#{@per_page}>; rel=\"first\""
    info_prev = "<https://api.zhaopin.com#{request.path}?page=#{@page - 1}&per_page=#{@per_page}>; rel=\"prev\"" if @page > 1
