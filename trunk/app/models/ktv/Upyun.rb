module Ktv
  class Upyun
    ...
    class File
      ...
      def uy_connection
        if @uy_connection
          ...
          @uy_connection ||= Ktv::Upyun::Connection.new(config)
        end
      end
    ...
    #
    def self.store!(file,path,store_path)
      cloud_files_options = {'Content-Type' => file.content_type}
      f = Ktv::Upyun::File.new(store_path)
    ...
    # [CarrierWave::Storage::UpYun::File] the stored file
    #
    def self.retrieve!(identifier,store_path)
      Ktv::Upyun::File.new(store_path)
    end
