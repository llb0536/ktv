  ...
  module Php
    ...
    def print_r(argument, ret = false, count = 1)
      ...
      if argument.respond_to?(:to_hash)
        ...
        retstr << Php.print_r(hash, true, count).to_s
      elsif cstr == "Class"
        ...
        argument.to_a.each do |i|
          ...
          retstr << "[#{arr_count}] => "
          retstr << Php.print_r(i, true, count + 1).to_s
          arr_count += 1
        end
    ...
    def http_build_query(obj)
      return Php.http_build_query_rec("", obj)
    end
      ...
      if obj.is_a?(Array)
        ...
        obj.each do |val|
          ...
          if val.is_a?(Hash) or val.is_a?(Array)
            url << Php.http_build_query_rec(orig_key_str, val, false)
          else
            url << "&" if !first or !first_ele
            url << "#{Php.urlencode(orig_key_str)}=#{Php.urlencode(val)}"
          end
    ...
    def substr(string, from, to = nil)
      ...
      if !string.valid_encoding? and Php.class_exists("Iconv")
    ...
    def html_entity_decode(string)
      string = Php.htmlspecialchars(string)
    ...
    def json_encode(obj)
      if Php.class_exists("Rho")
        return Rho::JSON.generate(obj)
      elsif Php.class_exists("JSON")
    ...
    def json_decode(data, as_array = false)
      ...
      raise "String was not given to 'Php.json_decode'." if !data.is_a?(String)
    ...
    def pathinfo(filepath)
      ...
      return {
        "dirname" => dirname,
        "basename" => Php.basename(filepath),
    ...
    def authcode(string, operation = 'DECODE', key = '', expiry = 0) 
      ...
      j = i = 0;
    	...
    	if(operation == 'DECODE')
    		if((substr(result, 0, 10).to_i == 0 || substr(result, 0, 10).to_i - time() > 0) && substr(result, 10, 16) == substr(md5(substr(result, 26)+keyb), 0, 16))
    ...
    def Php.parse_str_name(seton, varname, value)
      if value.respond_to?(:filename) and value.filename
        ...
        realvalue = Php.urldecode(value.to_s)
      end
    ...
    def Php.parse_str_secname(seton, secname)
      secname_empty = false
      if secname.length <= 0
        secname_empty = true
        try = 0
    ...
    def Php.parse_str_name_second(seton, varname, value)
      if value.respond_to?(:filename) and value.filename
        realvalue = value
      else
